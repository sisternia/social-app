  ...
  export default function EditProfileScreen() {
    ...
    const handleConfirm = async () => {
      try {
        ...
        const updates = {
          user_name: userName,
          user_dob: userDob
          ? `${userDob.getFullYear()}-${(userDob.getMonth() + 1)
              .toString()
    ...
    const pickBackground = async () => {
      ...
      if (!result.canceled && result.assets?.[0]) {
        try {
          const data = await uploadBackground(user_id as string, result.assets[0].uri);
    ...
    const pickAvatar = async () => {
      ...
      if (!result.canceled && result.assets?.[0]) {
        try {
          const data = await uploadAvatar(user_id as string, result.assets[0].uri);
